...
class TCP:
    ...
    def listen(self):
        ...
        self.face_radius_queue = deque(maxlen=20)
        ##############################################################################
        ...
        while self.running:
            try:
                ...
                if face_x == 0 or face_y == 0:
                    ...
                    if abs(face_x - prev_x) < tolerance or abs(face_y - prev_y) < tolerance:
                        yaw = map_value(face_x, 20, 620, -20, 20)
                        pitch = map_value(face_y, 20, 460, -20, 20)
                        fov = map_value(face_radius, 30, 100, 55, 80)
                        x = map_value(yaw, -20, 20, 100, 0)
                        y = map_value(pitch, -20, 20, 50, -50)
                        self.face_radius_queue.append(fov)
                        prev_x = face_x
                        prev_y = face_y

                        if len(self.face_radius_queue) == 20:
                            fov = sum(self.face_radius_queue) / len(self.face_radius_queue)
                            prev_data = {
                                "pitch": pitch,
                                "yaw": yaw,
                                "fov": fov,
